...
import type { NowData } from '@/lib/content'
import { useLanguage } from '@/context/LanguageContext'

/*
  Render half of NowStrip. Split out because the data comes from markdown read
  on the server, but the labels need the language context, which is client-side.
  NowStrip stays a server component and passes the data down.
*/
export default function NowStripView({ now }: { now: NowData }) {
  const { t } = useLanguage()
  ...
  const rows = [
    { key: 'building' as const, label: t.nowBuilding },
    { key: 'episode' as const, label: t.nowEpisode },
    { key: 'note' as const, label: t.nowNote },
  ]
      ...
      <div className="px-5 sm:px-6 py-14 sm:py-20">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-baseline justify-between mb-10">
            ...
            >
              {t.nowUpdated} {now.updated.toLowerCase()} ✓
            </span>
          </div>
